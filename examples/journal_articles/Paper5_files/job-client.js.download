var shared_errors = {}, shared_strings = {}, shared_http = {}, shared_requests = {}, shared_storage = {}, shared_numbers = {}, shared_collections = {}, shared_html = {}, shared_selection = {}, shared_templates = {}, shared_location = {}, shared_state = {}, shared_events = {}, shared_messaging = {}, config_config = {}, shared_scroll = {}, recommendation_recommendation = {}, job_job = {}, iframe_job_client = {};
...
job_job = function (exports, http_1, requests_1, config_1, scroll_1, state_1, events_1, selection_1, templates_1, errors_1, recommendation_1) {
  var JobApplication = function () {
    function JobApplication(state, recommendations, events, scroll) {
      var _this = this;
      ...
      this.events = events;
      this.scroll = scroll;
      this.assets = 'application/application.html';
      try {
        this.setup();
      } catch (e) {
        this.handle(e);
    ...
    JobApplication.prototype.handle = function (e) {
    ...
    JobApplication.prototype.setup = function () {
      var _this = this;
      ...
      var applicationElement = selection_1.get('.job', uptodateElement);
    ...
    JobApplication.prototype.loadAssets = function (done) {
      http_1.http(requests_1.Requests.get(this.state.config.host + this.assets), function (response) {
    ...
    JobApplication.prototype.display = function () {
      var _this = this;
      if (this.recommendations == null)
        return this.loadRecommendations(function () {
          return _this.display();
        });
      var parent = selection_1.get('.recommendations');
      var template = selection_1.get('template', parent);
      if (this.recommendations.length > 0) {
        document.body.style.display = 'block';
        this.recommendations.forEach(function (recommendation) {
          var instance = templates_1.Templates.clone(template);
          new recommendation_1.JobRecommendationRenderer(instance).render(recommendation);
          parent.appendChild(instance);
        });
        this.displayEvent();
      }
    };
    JobApplication.prototype.displayEvent = function () {
      var _this = this;
      ...
      var eventData = {
        'recommendation-ids': this.recommendations.map(function (r) {
          return r.id;
        }),
        'total-recommendations': this.recommendations.length
      };
      this.events.fire('inline', 'displays', 'job-recommendation', eventData);
    };
    JobApplication.prototype.loadRecommendations = function (done) {
      var _this = this;
      http_1.http(requests_1.Requests.jobRecommendations(this.state), function (response) {
        if (response.status != 200)
          return;
  ...
  return exports;
}(job_job, shared_http, shared_requests, config_config, shared_scroll, shared_state, shared_events, shared_selection, shared_templates, shared_errors, recommendation_recommendation);
...
iframe_job_client = function (exports, job_1, http_1, scroll_1, errors_1, messaging_1, selection_1, events_1) {
  var Client = function (_super) {
    ...
    function Client() {
      ...
      errors_1.Debug.log('JOB Client expecting to send and receive messages from ' + sourceOrigin);
      _super.call(this, window, sourceOrigin);
      this.sender = new messaging_1.MessageSender([window.parent], sourceOrigin);
      this.baseUrl = baseUrl;
      this.sender.send('job_state_request');
      selection_1.select(document.body).on('client_message', function (element, e) {
        var message = e.detail;
        _this.sender.send(message.name, message.body);
      }, true);
    }
    Client.prototype.job_state_response = function (state) {
      var _this = this;
      new job_1.JobApplication(state, null, new events_1.Events(state), new scroll_1.JobClientScrollListener(this.sender, this, state));
      window.setInterval(function () {
        _this.resizeIframe();
      }, 500);
    };
    Client.prototype.resizeIframe = function () {
      errors_1.Debug.log('JOB REC resize event from iframe');
      var application = selection_1.get('body', document);
  ...
  return exports;
}(iframe_job_client, job_job, shared_http, shared_scroll, shared_errors, shared_messaging, shared_selection, shared_events);
//# sourceMappingURL=job-client.js.map
